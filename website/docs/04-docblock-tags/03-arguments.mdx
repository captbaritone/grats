...
TypeScript default values will be used to infer the default value for the argument in the schema. Note that the default value must be parasable as a valid GraphQL literal.
...
```ts
/** @gqlField */
export function greeting(_: Query, name?: string = "Max"): string {
  return `Hello, ${name}`;
}
```
