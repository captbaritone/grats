...
TypeScript does not have a concept of interfaces implementing interfaces. However, GraphQL does. To declare that an interface implements another interface, you can use the `@gqlInterface` docblock tag in an interface declaration. The value following the tag should be a comma-separated list of interfaces that the interface implements.
