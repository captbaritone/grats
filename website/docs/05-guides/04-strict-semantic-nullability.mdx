...
The [current RFC](https://github.com/graphql/graphql-spec/pull/1065) for Strict Semantic Nullability proposes new Schema syntax for declaring output types which are only null in the case of error. However, new syntax requires support from many tools in the GraphQL ecosystem, including GraphQL servers, clients, and IDEs. In order to allow developers to experiment with Strict Semantic Nullability today, Grats uses a directive instead of new syntax. This is noisier, but allows developers to experiment with the idea without waiting for new syntax to be supported.
...
To see an full project example of Strict Semantic Nullability in action, check out the [example project](../05-examples/09-strict-semantic-nullability).
...
- Grats does not suppprt declaring items within a list as being semantically non-null
