...
export function validateGratsOptions(
  ...
  const result = _validateGratsOptions(options);
...
export function _validateGratsOptions(
  options: ts.ParsedCommandLine,
): Result<ParsedCommandLineGrats, string> {
  const gratsOptions = { ...(options.raw?.grats ?? {}) };
  for (const key of Object.keys(gratsOptions)) {
    if (!VALID_CONFIG_KEYS.has(key)) {
      // TODO: Suggest similar?
      return err(`Grats: Unknown Grats config option \`${key}\``);
    }
  }
