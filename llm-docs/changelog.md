...
-   **Features**
    ...
    -   Added [`--fix`](./getting-started/cli.md#automatic-fixes) flag to automatically fix fixable diagnostics. The CLI can now automatically apply fixes for common issues like incorrect casing in docblock tags and deprecated tag usage. The `--fix` flag works with both single builds and watch mode.
    ...
    -   Added [`tsClientEnums`](./getting-started/configuration.md#tsClientEnums) configuration option. When `tsClientEnums` is set, Grats will generate a TypeScript module containing all your GraphQL enum types as TypeScript enums for reuse in your client code.
-   **Performance**
...
This version introduces support for [defining directives](./docblock-tags/directive-definitions.md), and [annotating](./docblock-tags/directive-annotations.md) your schema with directives.
