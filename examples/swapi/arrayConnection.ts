...
import { Connection, Edge, PageInfo } from "./types/Film.js";
 ...
 * promised connection.
 */
// export async function connectionFromPromisedArray<T>(
...
function connectionFromArraySlice<T>(
  ...
  const edges: Edge<T>[] = slice.map((value, index) => ({
 ...
 */
// async function connectionFromPromisedArraySlice<T>(
//   dataPromise: Promise<ReadonlyArray<T>>,
//   args: ConnectionArguments,
